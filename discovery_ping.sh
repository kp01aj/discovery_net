...
# Uso: ./discovery_ping.sh [opciones] redes.txt
#
...
#
# Modos:
#   (por defecto)   -> fping, detener al primer host vivo en cada red
#   -m full         -> fping, revisar 100% de la red y listar todos los vivos
#   -e nmap         -> usar nmap -sn (ping scan) como motor alterno
#
# Parámetros:
#   -m {first|full}   Modo de escaneo (default: first)
#   -e {fping|nmap}   Motor (default: fping)
...
if [ "$ENGINE" = "fping" ] && ! have_cmd fping; then
  echo "❌ fping no está instalado. Instálalo: sudo apt-get install -y fping"; exit 1
fi
if [ "$ENGINE" = "nmap" ] && ! have_cmd nmap; then
  echo "❌ nmap no está instalado. Instálalo: sudo apt-get install -y nmap"; exit 1
fi
if ! have_cmd python3; then
  echo "❌ Python3 es necesario para expandir redes CIDR. Instálalo y reintenta."; exit 1
fi
...
# fping a 1 host (unicast)
fping_one() {
  local ip="$1"
  fping -r "$RETRIES" -t "$TIMEOUT_MS" "$ip" >/dev/null 2>&1
}

# nmap discovery (devuelve lista de vivos)
nmap_discovery() {
  local target="$1"
  # -sn ping scan; filtramos IPs
...
for objetivo in "${objetivos[@]}"; do
  ...
  if [[ "$objetivo" == */* ]]; then
    # ----- Red CIDR -----
    if [ "$ENGINE" = "nmap" ]; then
      echo "🔧 Método: nmap -sn (descubrimiento por ICMP/ARP/otros, sin puertos)"
      vivos=( $(nmap_discovery "$objetivo") )
      if [ "${#vivos[@]}" -gt 0 ]; then
        if [ "$MODE" = "first" ]; then
          echo "✅ Primer host vivo: ${vivos[0]} (red $objetivo)"
          alcanzadas+=("$objetivo (${vivos[0]})")
        else
          echo "✅ ${#vivos[@]} hosts vivos encontrados en $objetivo:"
          for ip in "${vivos[@]}"; do echo "  - $ip"; done
      ...
      if [ "${#hosts[@]}" -eq 0 ]; then
        echo "⚠️ CIDR inválido o sin hosts: $objetivo"
      ...
      if [ "$MODE" = "first" ]; then
        ...
        if [ "${#vivos_list[@]}" -gt 0 ]; then
          echo "✅ ${#vivos_list[@]} hosts vivos en $objetivo:"
          for ip in "${vivos_list[@]}"; do echo "  - $ip"; done
    ...
    if is_ip "$objetivo"; then
      ...
      if [ "$ENGINE" = "nmap" ]; then
        ...
        if [ "${#vivos[@]}" -gt 0 ]; then
          ...
          echo "❌ No responde $objetivo"; no_alcanzadas+=("$objetivo")
        fi
      else
        if fping_one "$objetivo"; then
          printf "\n"; echo "✅ Host vivo $objetivo"; alcanzadas+=("$objetivo")
        else
          printf "\n"; echo "❌ No responde $objetivo"; no_alcanzadas+=("$objetivo")
        fi
      ...
      echo "⚠️ Línea inválida (ni IP ni CIDR): $objetivo"
      no_alcanzadas+=("$objetivo (inválido)")
    fi
  fi
done
